...
export function Main(cnf: Cnf, deps: Deps) {
  const axiosError = (e: AxiosError) =>
    (() => {
      ...
      return [(d as any).code || r.status, (d as any).message || JSON.stringify(d)];
    })().join("\t");
  ...
  if (retrys) {
    for (const x of retrys as VERBS[]) {
      if (typeof instance[x] !== "function") continue;
      instance[x] = retryAble(instance[x], retryTimes, retryIntervalMS) as any;
    }
  }

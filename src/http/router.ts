...
import { HttpCodes, Domain, Err } from "./defines";

export function Main(utils: ReturnType<typeof Utils>) {
  const { ucwords, makeParams, makeProfile, outputCSV, jsonSchema2Swagger } = utils;

  return (server: restify.Server, httpCodes: HttpCodes, makeProfileHook?: Function) => {
     ...
     *  apisRoute 所有api地址查看
     */
    function Router(domain: Domain, apisRoute: string, swagger: [string, any] = ["", {}]) {
